    ...
    <div class="store-content page-cart">
      <div class="gray-box">
        ...
        <div v-if="cartList.length">
          <div class="ui-cart">
            <div>
              ...
              <!--购物车内容-->
              <div class="cart-table" v-for="(item,index) of cartList" :key="index">
        ...
        <div v-else style="padding:50px">
          ...
          <p style="text-align: center;padding: 20px;color: #8d8d8d">你的购物车是空的，赶快前去选购吧</p>
          <div style="text-align: center">
            <router-link to="/goods">
              <y-button text="前往选购" style="width: 150px;height: 40px;line-height: 38px;font-size: 15px;" classStyle="main-btn"></y-button>
            </router-link>
          </div>
        </div>
      </div>
...
<script>
  ...
  import { cartEdit, editCheckAll, cartDel } from '/api'
  export default {
    data () {
      return {
        userId: 0,
        checkoutNow: '前往购买',
        submit: true //支付按钮是否可按
      }
    },
    computed: {
      ...mapState(['cartList']),
    ...
    methods:{
      ...mapMutations(['INIT_BUYCART', 'EDIT_CART','SHOW_CART']),
      goodsDetails (id) {
        window.open(window.location.origin + '#/goodsDetails?productId=' + id)
      },
      editCheckAll () {
        //如果是‘全选’的话变成‘全不选’，如果是‘全不选’的话变成‘全选’checked为1表示选中
    ...
    mounted() {
      ...
      this.INIT_BUYCART();
      this.SHOW_CART(false);//打开时会弹出购物信息，关闭它
    },
    components: {
      YHeader,
      YFooter,
